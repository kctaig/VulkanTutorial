...
#define GLM_FORCE_DEPTH__ZERO_TO_ONE
#include <glm/glm.hpp>
...
class HelloTriangleApplication {
  ...
  private:
    ...
    void createGraphicsPipeline() {
        ...
        auto bingdingDescription = Vertex::getBindingDescription();
        auto attributeDescriptions = Vertex::getAttributeDescriptions();

        VkPipelineVertexInputStateCreateInfo vertexInputInfo{};
        vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
        vertexInputInfo.vertexBindingDescriptionCount = 1;
        vertexInputInfo.pVertexBindingDescriptions = &bingdingDescription;
    ...
    void createCommandPool() {
        ...
        if (vkCreateCommandPool(device, &graphicsPoolInfo, nullptr, &graphicsCommandPool) !=
            VK_SUCCESS) {
        ...
        if (vkCreateCommandPool(device, &transferPoolInfo, nullptr, &transferCommandPool) !=
            VK_SUCCESS) {
    ...
    void createTextureImage() {
        ...
        stbi_uc* pixels = stbi_load(
            getAbsPath(TEXTURE_PATH).string().c_str(), &texWidth, &texHeight, &texChannels,
            STBI_rgb_alpha
        );
        ...
        createImage(
            texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB,
            VK_IMAGE_TILING_OPTIMAL,
    ...
    void loadModel() {
        ...
        if (!tinyobj::LoadObj(
                &attrib, &shapes, &materials, &warn, &err, getAbsPath(MODEL_PATH).string().c_str()
            )) {
    ...
    void createIndexBuffer() {
        VkDeviceSize bufferSize = sizeof(indices[9]) * indices.size();
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vkUpdateDescriptorSets(
                device, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0,
                nullptr
            );
        }
    }
    ...
    uint32_t findMemotyType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    ...
    VkCommandBuffer beginSingleTimeCommands(VkCommandPool commandPool) {
        ...
        allocaInfo.commandPool = commandPool;
    ...
    void endSingleTimeCommands(
        VkCommandBuffer commandBuffer, VkQueue queue, VkCommandPool commandPool
    ) {
    ...
    void drawFrame() {
        ...
        VkResult result = vkAcquireNextImageKHR(
            device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE,
            &imageIndex
        );
